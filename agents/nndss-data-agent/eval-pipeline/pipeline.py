"""NNDSS Capability Evaluation Pipeline (KFP).

Runs the NNDSS data agent against a question corpus, judges responses on
7 capability dimensions, logs results to MLflow, and produces a scorecard.

Pipeline stages:
  1. Load questions (from PVC)
  2. Run agent (MCP server via MCPAgentBlock)
  3. Triage (extract reasoning, answer, tool interactions)
  4. Merge with gold standards
  5. Capability judge (7 dimensions)
  6. Summary (per-capability scores)
  7. Log to MLflow (eval dataset + metrics + artifacts)
...
from components import (
    load_questions,
    triage,
    merge_gold_standards,
    summary,
    log_to_mlflow,
)


DEFAULT_MCP_SERVER_URL = "http://nndss-mcp-server:9090/mcp/"
...
@dsl.pipeline(
    name="nndss-capability-eval",
    description=(
        "NNDSS capability evaluation pipeline. Runs agent against Australian "
        "disease surveillance questions, judges on 7 capability dimensions, "
        "logs results to MLflow."
    ),
)
def eval_pipeline(
    limit: int = 0,
    run_name: str = "nndss-eval-v1",
    input_file: str = "nndss-questions.jsonl",
    ...
    # Judge model is pinned — keep the same checkpoint across A/B runs
    # so scores remain comparable. Only the agent model varies.
    judge_task = sdg_flow(
    ...
    # --- Stage 7: Log to MLflow ---
    mlflow_task = log_to_mlflow(  # noqa: F841
        judged_data=judge_task.outputs["output_artifact"],
        questions_data=load_task.outputs["output_data"],
        run_name=run_name,
    )
    use_secret_as_env(
        mlflow_task,
